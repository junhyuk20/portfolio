...
public class PageNavigator {
	//페이지 관련 정보 
	private int countPerPage;		//페이지당 글목록 수			10개씩  
	private int pagePerGroup;		//그룹당 페이지 수  			5 (1 2 3 4 5 - 0그룹), (6 7 8 9 10 - 1그룹)
	private int currentPage;		//현재 페이지 (최근 글이 1부터 시작) 1페이지
	private int totalRecordsCount;	//전체 글 수가   				78개
	private int totalPageCount;		//전체 페이지 수 				8페이지
	private int currentGroup;		//현재 그룹 (최근 그룹이 0부터 시작) 3번째에서 1페이지라고 가정햇으니 내 그룹은 0
	private int startPageGroup;		//현재 그룹의 첫 페이지			 1페이지
	private int endPageGroup;		//현재 그룹의 마지막 페이지		 5페이지
	private int startRecord;		//전체 레코드 중 현재 페이지 첫 글의 위치 (0부터 시작)		0 (현재 1페이에서 첫 번째 글 위치는 0이다) 
	...
	public PageNavigator(int countPerPage, int pagePerGroup, int currentPage, int totalRecordsCount) {
		...
		//전체 페이지 수		(78 + 10 -1) / 10 = 8
		//만약 (100 + 10) / 10 = 11패이지가 나온다 그러므로 -1해 줘야 한다
